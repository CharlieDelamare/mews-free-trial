...
import { autoCheckout } from '@/lib/control-centre/operations-service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const log = await prisma.controlCentreLog.create({
      data: { logType: 'auto_checkout', enterpriseId, status: 'processing' },
    });

    const work = autoCheckout(token, log.id)
    ...
    return NextResponse.json({ success: true, logId: log.id, message: 'Auto-checkout started.' });
  } catch (e) {
